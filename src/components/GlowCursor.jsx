...
/**
 * 鼠标发光拖尾特效(来自 React Bits 的 GlowCursor 组件,做了两处改动)
 * ------------------------------------------------------------
 * 1. 原版组件需要把网站内容包在它里面(children),这里改成了
 *    一个"铺满整个屏幕、不遮挡点击"的固定图层,直接放在 App 最外层
 *    就能让全站都有这个跟随鼠标的发光拖尾,不用改动其他任何模块。
 * 2. 颜色换成了网站自己的红色/暖白色调,而不是原版的青色/紫色,
 *    这样和整站黑白灰 + 红色的风格保持一致。
 *
 * 想关掉这个特效:去 App.jsx 里把 <GlowCursor /> 这一行删掉或注释掉即可。
 * 想调颜色/粗细/亮度:改下面 DEFAULT_CONFIG 里的数值,不用碰下面的逻辑代码。
 */

const MAX_POINTS = 64;

const DEFAULT_CONFIG = {
  color: "#ff3c20", // 拖尾最亮的头部颜色 —— 用网站的主红色
  secondaryColor: "#f4f3ef", // 拖尾尾部颜色 —— 用网站的暖白色
  trailLength: 34,
  trailWidth: 6,
  trailTaper: 0.82,
  followSpeed: 0.16,
...
export default function GlowCursor(props) {
  const containerRef = useRef(null);
  const canvasRef = useRef(null);
  const propsRef = useRef({});

  propsRef.current = { ...DEFAULT_CONFIG, ...props };

  useEffect(() => {
    // 尊重"减少动态效果"的系统设置,避免对敏感用户造成不适
    ...
    // 这里跟原版不同:改成跟随"整个浏览器窗口"的大小,而不是某个容器的大小,
    // 因为这个特效是铺在全站上面的固定图层。
    const resize = () => {
    ...
    // 这里也跟原版不同:直接监听整个 window 的鼠标位置(而不是某个容器),
    // 这样即使发光图层本身设置了"不挡点击"(pointer-events: none),
    // 也依然能实时拿到鼠标坐标——页面上的按钮、链接完全不受影响,照常可以点击。
  ...
  return (
    <div ref={containerRef} className="glow-cursor-overlay" aria-hidden="true">
      <canvas
        ref={canvasRef}
        className="glow-cursor__canvas"
        style={{ mixBlendMode: propsRef.current.blendMode }}
      />
    </div>
  );
}
